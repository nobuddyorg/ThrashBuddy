...
const TEST_FILE_PATH = path.resolve(__dirname, "../../k6/example", TEST_FILE_NAME);
...
test("open Grafana and login", async ({ page }) => {
    ...
    await newPage.getByTestId("data-testid Username input field").fill(process.env.USERNAME_TOOLS || "");
    await newPage.getByTestId("data-testid Password input field").fill(process.env.PASSWORD_TOOLS || "");
